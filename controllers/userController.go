...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//convert json data coming from postman to something that golang understand
		...
		//check if email has already been used by another user or not
		...
		//check if phone number has already been used by other user
		...
		//generate token and refresh token (generateAllTokens from helpers)
		token, refreshToken, _ := helper.GenerateAllTokens(*user.Email, *user.First_name, *user.Last_name, user.User_id)
		user.Token = &token
		user.Refresh_Token = &refreshToken
		//if all ok, insert this new user in the user collection
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//convert the login data from postman to golang understandable format
		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		//find the user with the email and see if that user even exists
